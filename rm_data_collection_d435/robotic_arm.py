...
class Arm():
    ...
    def init_first(self):
        ...
        result = self.pDll.Get_DH_Data(self.nSocket, ctypes.byref(lsb), ctypes.byref(lse),
                                       ctypes.byref(lew), ctypes.byref(lwt), ctypes.byref(d3))
        if result == 0:
            logger_.info(
                f'DH 参数获取成功: lsb={lsb.value}, lse={lse.value}, lew={lew.value}, lwt={lwt.value}, d3={d3.value}')
        else:
            logger_.info('获取 DH 参数失败')
